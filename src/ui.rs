use crate::app::{App, Card, GameResult, Hand, PlayerStatus, Rank, Suit};
...
fn card_face(rank: &str, suit: &str, spacing: &str) -> String {
    return format!(
        "\n╭───────────╮\n\
         │{rank}{spacing}         │\n\
         │           │\n\
...
fn render_title<B: Backend>(f: &mut Frame<B>, area: Rect) {
    ...
    let deck_logo = Paragraph::new(DECK).alignment(Alignment::Left);
    f.render_widget(deck_logo, caption_chunks[0]);

    let caption = Paragraph::new(TITLE).alignment(Alignment::Left);
    ...
    if num_cards > 0 {
        ...
        for (i, card) in hand.cards.iter().enumerate() {
            ...
            render_card(
                ...
                card,
                color,
                if is_first { Some(name) } else { None },
            );
        }
    }
}
...
fn render_status<B: Backend>(f: &mut Frame<B>, area: Rect, app: &App) {
    if !app.game_running {
        let status_bar = Paragraph::new(NEW_GAME_TEXT).alignment(Alignment::Center);
        f.render_widget(status_bar, area);
    } else if app.player_status == PlayerStatus::Draw {
        let status_bar = Paragraph::new("Draw card [d] or stand [s]?").alignment(Alignment::Center);
        f.render_widget(status_bar, area);
    } else if let Some(game_result) = &app.result {
        let text = match game_result {
            GameResult::Won => "Congrats, you won the game!",
            GameResult::Lost => "Sorry, you lost the game!",
            GameResult::Push => "Nobody won, it's a push.",
        };
        let status_bar =
            Paragraph::new(format!("{}\n{}", text, NEW_GAME_TEXT)).alignment(Alignment::Center);
        f.render_widget(status_bar, area);
    }
}
